...
use vars qw ($perl $getApFlash $updateApconfig $searchAp $fetchPreset $getApGroupList $joinApGroup $login $rebootAp $getTemplate $getProperty $provisionSubmitation $log_path $session_path);
...
my $session = new CGI::Session(undef, undef, {Directory=>$session_path}) or die CGI::Session->errstr;
...
if(!$username){
    ...
    <center><h2>Cisco AP Provisioning Tool</h2>
...
EndHTML
    ...
    print start_form(-id => 'loginForm', -method=>'POST', -action=>"http://$host/cgi-bin/appt/$script_name", -onSubmit=>"javascript:return login('/cgi-bin/appt/$login', '$client_ip')"), "<br>",
    ...
    if (param('ap_mac') ne "FF:FF:FF:FF:FF:FF") {
        ...
        $search_ap .= " $ap_mac_dec $username";
    } else {
     $search_ap .= " '' $username";
    }
    my $result = `$search_ap`;
    my ($error, $result_data) = split(";", $result, 2);
    print "<div class=\"loginbar\"><div class=\"logout light\" onclick=\"logout('http://$host/cgi-bin/appt/$script_name');\">logout: $username</div></div>";
...
EndHTML
        ...
        print "</div>";
        print start_form(-id => 'loginForm', -method=>'POST', -action=>"http://$host/cgi-bin/appt/$script_name", -onSubmit=>"javascript:return checkMACAddress();"), "<br>", 
...
EndHTML
        print "<legend id='info_header'><div onclick=\"flashAp('/cgi-bin/appt/$getApFlash', '$staging_ap_info[2]')\">$staging_ap_info[1]</div></legend>";
        print <<EndHTML;
                <table id='template_table'>
                    <tr class='template_property'> 
                        ...
                        <td>
EndHTML
        print popup_menu( -name => 'property', -id => 'property', -value =>\@property_menu, -onchange=>"loadTemplate('/cgi-bin/appt/$getTemplate', '/cgi-bin/appt/$getProperty', '$staging_ap_info[1]', '$staging_ap_info[2]')");
        print <<EndHTML;
            </td>
            </tr>
        ...
        <table id='comfirm_table' style='display:none;'>
                <caption> Confirm Informarion </caption>
            <tr>
                <td></td>
            ...
            <tr>   
EndHTML
            print "<td><button onclick=\"submitChange('/cgi-bin/appt/$provisionSubmitation');\">Submit</button></td>";
            print <<EndHTML
                <td></td>
                <td>
